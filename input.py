...
def parse_example(example_proto):
    ...
    Parameters:
    examples_proto: The thing to parse.

    Returns: The parsed thing. Note: This is always channels_first!
    """
    # TODO look into better ways of doing this (SequenceExample SUCKS, maybe try bytes) (compare speeds!!)
    ...
    dense_seq = tf.reshape(tf.sparse_to_dense(sparse_seq.indices, sparse_seq.dense_shape, sparse_seq.values),
                           shape)
    # add fake channel axis in any case
    return tf.expand_dims(dense_seq, axis=0), tf.cast(parsed_features["label"], tf.int32)
