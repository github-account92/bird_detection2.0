...
parser = argparse.ArgumentParser(description="Train a CNN to recognize bird song.")
parser.add_argument("data_path",
                    help="Base path to tfrecords data (without train/dev/extension!).")
parser.add_argument("model_dir",
                    help="Path to store checkpoints etc.")
parser.add_argument("-a", "--act",
                    default="relu",
                    choices=["relu", "elu"],
                    help="Which activation function to use. Can be one of 'relu' (default),"
                         "or 'elu'.")
...
parser.add_argument("-E", "--eval_freq",
                    type=int,
                    default=1000,
                    help="How often to evaluate. Default: Every 1000 steps. Doing this too "
                         "often may result in significant slowdown of the overall process.")
parser.add_argument("-F", "--format",
                    default="channels_first",
                    choices=["channels_first", "channels_last"],
                    help="Data format. Either 'channels_first' (default, recommended for GPU) "
                         "or 'channels_last', recommended for CPU.")
parser.add_argument("-M", "--mode",
                    default="raw",
                    choices=["raw", "spectrum", "mel"],
                    help="What mode to run (what kind of input). Can be one of 'raw' (default), "
                         "'spectrum' or 'mel'")
...
parser.add_argument("-V", "--vis",
                    action="store_true",
                    help="If set, add more visualizations (inputs and filter maps).")
...
# note: a logging hook for the cost as well as a step counter hook seem to be
# created automatically
logging_hook = tf.train.LoggingTensorHook({"eval/accuracy": "eval/batch_accuracy"},
                                          every_n_iter=100,
                                          at_end=True)
steps_taken = 0
